...
#include "ConnectionXillybus/ConnectionXillybusQSpark.h"
...
LMS7_Device* LMS7_Device::CreateDevice(lime::IConnection* conn)
{
    LMS7_Device* device;
    if (conn != nullptr)
    {
        auto info = conn->GetDeviceInfo();
        if (info.deviceName == "QSpark")
            device = new LMS7_QSpark();
        else device = new LMS7_Device();
...
LMS7_Device::LMS7_Device() : connection(nullptr), lms_chip_id(0)
{
    this->tx_channels.resize(2);
    this->rx_channels.resize(2);
    this->lms_list.resize(1);
}
...
void LMS7_Device::_Initialize(lime::IConnection* conn)
{
    ...
    this->lms_list.resize(0);               
    this->lms_list.resize(this->GetNumChannels()/2);
...
int LMS7_Device::SetLPF_Fixed(bool tx,size_t chan, LPF_FixedBW bandwidth)
{
    ...
    return lms->TuneTxFilterFixed(bw);
}
...
int LMS7_Device::SetNormalizedGain(bool dir_tx, size_t chan,double gain)
{
    ...
    {
        int g = (63*gain+0.49);
        if (lms->Modify_SPI_Reg_bits(LMS7param(CG_IAMP_TBB), g, true) != 0)
            return -1;
    }
    else
    {
...
double LMS7_Device::GetNormalizedGain(bool dir_tx, size_t chan)
{
    ...
    if (dir_tx)
    {
        double ret = lms->Get_SPI_Reg_bits(LMS7param(CG_IAMP_TBB), true);
        ret /= 63.0;
        return ret;
    }
    else
...
int LMS7_Device::Init()
{
    ...
    for (unsigned i = 0; i < lms_list.size(); i++)
    {
        ...
        if (lms->Modify_SPI_Reg_bits(LMS7param(EN_ADCCLKH_CLKGN), 0) != 0)
            return -1;
        ...
        lms->Modify_SPI_Reg_bits(LMS7param(DC_BYP_RXTSP), 1);
        lms->Modify_SPI_Reg_bits(LMS7param(GC_BYP_RXTSP), 1);
        lms->Modify_SPI_Reg_bits(LMS7param(PH_BYP_RXTSP), 1);

        lms->Modify_SPI_Reg_bits(LMS7param(GFIR1_BYP_TXTSP), 1);
        lms->Modify_SPI_Reg_bits(LMS7param(GFIR2_BYP_TXTSP), 1);
        lms->Modify_SPI_Reg_bits(LMS7param(GFIR3_BYP_TXTSP), 1);
        lms->Modify_SPI_Reg_bits(LMS7param(CMIX_BYP_TXTSP), 1);
        lms->Modify_SPI_Reg_bits(LMS7param(DC_BYP_TXTSP), 1);
        lms->Modify_SPI_Reg_bits(LMS7param(GC_BYP_TXTSP), 1);
        if (lms->Modify_SPI_Reg_bits(LMS7param(PH_BYP_TXTSP), 1)!=0)
            return -1;
...
int LMS7_Device::ProgramMCU(const char* data, size_t len, lms_target_t target,lime::IConnection::ProgrammingCallback callback)
{
    ...
    mcu.Initialize(this->connection);
...
LMS7_QSpark::LMS7_QSpark() : LMS7_Device()
{
    this->streamPort=nullptr;
}

size_t LMS7_QSpark::GetNumChannels(const bool tx) const
{
    return 4;
};

int LMS7_QSpark::SetConnection(lime::IConnection* conn)
{
    LMS7_Device::SetConnection(conn);
    if (this->streamPort != nullptr)
        delete this->streamPort;
    if (conn != nullptr)
    {
        lime::ConnectionXillybusQSpark* obj = dynamic_cast<lime::ConnectionXillybusQSpark*>(conn);
        this->streamPort = new lime::ConnectionXillybusQSpark(*obj);
    }
    else this->streamPort = nullptr;
    return 0;
}

lime::IConnection* LMS7_QSpark::GetConnection(unsigned chan)
{
    if (chan & 0x2)
